...
ggplot() +
  ...
  scale_fill_gradientn(colours = warming_bars_palette,
                       ...
                       guide = guide_colourbar(nbin = 50,
                                               ticks.colour = "black",
                                               ticks.linewidth = 0.5,
                                               frame.colour = "grey50",
                                               raster = TRUE)) +
  labs(title = "DROUGHT CONDITIONS IN US SOUTHWEST: JUNE 2022",
       subtitle = "June 2022 was the 10th worst drought month on record since 2000",
       caption = "Data: www.drought.gov | Plot: @ldbailey255") +
  theme_void() +
  theme(legend.position = "bottom", legend.title = element_blank(),
        legend.text = element_text(colour = "white", family = "Miki", size = 10),
        legend.key.width = unit(3, units = "cm"),
        plot.background = element_rect(fill = "grey10", colour = NA),
        plot.margin = margin(t = 20, r = 20, b = 20, l = 20),
        plot.title = element_text(colour = "white", size = 20, hjust = 0, family = "Alfa_Slab_One",
                                  margin = margin(b = 5)),
...
ggsave(here(glue::glue("./plots/2022/Week24_map_staticJun2022.png", date = date)),
...
### gif small enough for Twitter (<=15MB)
...
purrr::map(.x = dates, .f = function(date){
  ...
  sw_drought <- ggplot(data = plot_data) +
    ...
    geom_segment(aes(x = date, xend = date,
                     y = 1.5, yend = 1),
                 arrow = arrow(length = unit(1, units = "mm")), colour = "white") +
    scale_fill_gradientn(colours = warming_bars_palette,
                         #These are extended so that DSCI can be compared to the county wide data on the map
                         limits = c(-350, 350),
                         breaks = c(-350, 0, 350)) +
    ...
    theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.text.x = element_text(colour = "white", size = 7, family = "Miki"),
          axis.ticks = element_blank(),
          axis.title = element_blank(),
          legend.position = "none",
          plot.background = element_rect(fill = "grey10", colour = NA),
  ...
  ggplot() +
    ...
    geom_richtext(aes(x = 1 - 0.04, y = 0.15,
                  ...
                  size = 3) +
    labs(title = "DROUGHT IN THE US SOUTHWEST IS THE\nWORST IN OVER 2 DECADES",
         subtitle = paste(worst_droughts, "of the 20 worst drought months have occurred since 2020"),
         caption = "Data: www.drought.gov | Plot: @ldbailey255") +
    theme_void() +
    theme(plot.background = element_rect(fill = "grey10", colour = NA),
          plot.margin = margin(t = 0, r = 50, b = 0, l = 50),
          plot.title = element_text(colour = "white", size = 10, hjust = 0, family = "Alfa_Slab_One"),
          plot.subtitle = element_text(colour = "white", size = 9, hjust = 0, family = "Miki"),
          plot.caption = element_text(colour = "white", hjust = 1, family = "Cutive_Mono"))
  
  ggsave(here(glue::glue("./plots/2022/Week24_gif/Week24_map_{date}.png", date = date)),
         height = 4.5, width = 6.5, dpi = 125)
...
## save to disk
image_write(image = img_animated,
            path = here("./plots/2022/Week24_map_twitter.gif"))
