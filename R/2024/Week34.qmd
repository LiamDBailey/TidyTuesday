...
## Shortern name of Sophia-Dorothea (for easier plotting)
marriages_postnorman$consort_name[marriages_postnorman$consort_name == "Sophia Dorothea of Brunswick-Lueneburg-Celle"] <- "Sophia Dorothea of Celle"
```
...
houses_latlon <- read.csv(here("./data/week34_royalmarriage/houses.csv")) |> 
  ...
  ## Filter out English and American marriages
  filter(!english & modern_country != "America")

houses_latlon_lines <- houses_latlon |> 
  ## For consorts from exactly the same location jitter slightly
  group_by(lon, lat) |>
  mutate(lon = lon + (seq(1, n(), 1)-1)*0.15,
         lat = lat + (seq(1, n(), 1)-1)*0.15) |>
  ungroup()

## Create a linestring using spherical geometry (i.e. greater circle)
linestring <- purrr::map2(.x = houses_latlon_lines$lat, .y = houses_latlon_lines$lon, function(lat, lon){
  
  sf::st_as_sfc(geosphere::gcIntermediate(c(lon, lat), c(-0.1276, 51.5072), 
...
## Add this back to our data, transforming it into an sf object
houses_latlon_sf <- bind_cols(linestring, houses_latlon_lines)
```
...
## Convert our line data into the correct CRS
house_latlon_trans_lines <- houses_latlon_sf |> 
  sf::st_transform(crs)

## Also create data for the location of the house crests
## NOTE: This data is *not* jittered to remove overlap
house_latlon_trans_points <- houses_latlon |> 
  sf::st_as_sf(coords = c("lon", "lat"),
               crs = "EPSG:4326") |> 
  sf::st_transform(crs)

house_latlon_trans_points <- bind_cols(house_latlon_trans_points,
                                       sf::st_coordinates(house_latlon_trans_points))
...
Create the raw globe (without anything else atm)
...
new_map <- raw_map +
  ## Draw all lines
  geom_sf(data = house_latlon_trans_lines,
          colour = "white", linewidth = 0.15) +
  geomtextpath::geom_textsf(data = house_latlon_trans_lines |> 
                              filter(label & vjust == 0.25),
                            aes(label = consort_name),
                            colour = "white", size = 1.5,
                            hjust = 0.075, family = "Luminari",
                            linewidth = 0, vjust = 0.25) +
  geomtextpath::geom_textsf(data = house_latlon_trans_lines |> 
                              filter(label & vjust == 1),
                            aes(label = consort_name),
                            colour = "white", size = 1.5,
                            hjust = 0.075, family = "Luminari",
                            linewidth = 0, vjust = 1)
...
```{r}
map <- new_map +
  ggimage::geom_image(data = house_latlon_trans_points,
                      aes(x = X, y = Y, image = crest), size = 0.015) +
...
plot_data <- houses_latlon |> 
  filter(!modern_country %in% c("England", "America")) |> 
  ## For each country...
